...
class TestCreateGenre:
    def test_create_genre_with_associated_categories(self):
        ...
        category_repository.save(
            Category(name="Category 1", description="Category 1 description")
        )
        category_repository.save(
            Category(name="Category 2", description="Category 2 description")
        )
        ...
        assert created_genre.categories == {
            cat.id for cat in category_repository.list()
        }
    ...
    def test_create_genre_with_inexistent_categories_raise_an_error(self):
        ...
        with pytest.raises(
            ...
            use_case.execute(
                CreateGenre.Input(name="Genre 1", categories={uuid.uuid4()})
            )
