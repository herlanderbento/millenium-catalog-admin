...
class TestCreateGenre:
    def test_when_categories_do_not_exits_the_raise_related_categories_not_found(
        ...
        with pytest.raises(
            ...
            use_case.execute(
                CreateGenre.Input(name="Genre 1", categories={category_id})
            )

        assert str(category_id) in str(exc_info.value)  # type: ignore
    ...
    def test_when_created_genre_is_valid_and_categories_exist_then_save_genre(
        ...
        output = use_case.execute(
            CreateGenre.Input(
                name="Romance", categories={documentary_category.id, movie_category.id}
            )
        )
    ...
    def test_create_genre_without_categories(self, mock_genre_repository, mock_category_repository_with_categories):
        use_case = CreateGenre(
            genre_repository=mock_genre_repository,
            category_repository=mock_category_repository_with_categories,
        )
